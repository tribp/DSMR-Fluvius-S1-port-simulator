...
For testing or debugging applications that read those S1 telegrams it would bet nice to:

- control the speed of the telegrams
- control the content of the fields of the telegrams.

Since a 'ESP32' microcontroller would be the perfect candidate for **reading** the S1 messages, it made sense to also use it to make a **'Smart meter S1 similator'**

## 2 Intro

### 2.1 S1 Interface

- RS422: This a 'balanced signal (+ & -) with same voltage levels as RS 485. The difference with RS485 is that it is not a 'bus' with addressable devices like RS485, but only for 'point to point'. Advandage is that we can use a standard RS485 level converter like [RS485 module](https://nl.aliexpress.com/item/32682660246.html?spm=a2g0s.9042311.0.0.1cde4c4dELnNA3)
- RJ12
...
- Hardware UART of ESP32 is used.
...
The standard [source file]("./src/esp32_DSMR_S1_tx.ino") sends a 'perfect 50Hz - 230V RMS - sinewave' with a 'real-life' smart meter interval at 384us.
...
- Change **signal**
  ...
  - you can manually modify:
    ...
    - add harmonics to test your DFT-fraq analysis- at the receiver.
